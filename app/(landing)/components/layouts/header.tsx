...
const Header = () =>{
    ...
    <header className="fixed w-full z-20 backdrop-blur-3xl bg-white/50">
        <div className="flex justify-between gap-10 container mx-auto px-40 mt-4">
        <Link href="/">
        <Image 
        src="/image/Logo.svg" 
        alt="sporton logo" 
        ...
        {items.length ? (<div className="bg-primary rounded-full w-3.5 h-3.5 absolute top-4 right-[-2] text-[10px] text-white text-center" >
            {items.length}
        </div>
        ):(
            <></>
